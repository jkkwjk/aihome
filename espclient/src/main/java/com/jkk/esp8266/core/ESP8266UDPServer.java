...
import com.jkk.esp8266.exception.AppNameRepeatException;
...
public class ESP8266UDPServer {
	private static Logger logger = LoggerFactory.getLogger(ESP8266UDPServer.class);
			...
			try {
				...
				while (true) {
					...
					if (this.handle(data, packet.getAddress(), packet.getPort())) {
						...
						if (pos == -1) {
							// 发送的数据包没有appId
							logger.warn("Don't send appId, receiveData = {}", data);
						...
						if (handleProcessList == null) {
							// appId 不存在
							logger.warn("Can't find appId {}, receiveData = {}", appId, data);
						...
						while (i < handleProcessList.size()) {
							...
							if (! handleData.equals(result.getNewData())) logger.info("data was changed, {} in handle {}", handleData, i);
				...
				logger.error("server stop" + e.getMessage());
			}
		}).start();
	}
